...
public class BookTest {
	...
	public static void main(String[] args) {
		...
		BookManagerImpl.getInstance().insertBook(new novel("1","로미오와 줄리엣","셰익스피어","최예준",15000.0,"A"));
		BookManagerImpl.getInstance().insertBook(new Magzine("2","햄릿","셰익스피어","최예준",35000.0,"B"));
		
		System.out.println(BookManagerImpl.getInstance().getNumberOfBooks());//현재 책 갯수
		
		Book[] bk = BookManagerImpl.getInstance().getAllBook(); // 현재 책정보
		for(Book i : bk) {
