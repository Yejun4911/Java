...
public class BankTest {
	...
	public static void addCustomer() {
		...
		String city = sc.next();
		sc.nextLine();
		Address ac = new Address(zipCode,region,city);
		Customer customer = new Customer(name,ssn,ac);//2. 입력받은 값으로Customer 객체를 생성
		//3. 배열에 추가함
	...
	public static void createAccount() {
		...
		int balance = 0;
		if(BankService.getInstance().getCustomer(ssn)!=null) {//1. 입력받은 ssn에 해당하는 고객이 없지 않다면 통장개설을 하기위한 잔액을 먼저 입력받고 통장을 개설함
			
			System.out.println("잔액 입력>");
			balance = sc.nextInt();
			BankService.getInstance().getCustomer(ssn).setAccount(new Account(balance));

		}else return; //2. 만약에 회원이 없다면 return시킴
	...
	public static void showCustomer() {
		...
		if(BankService.getInstance().getCustomer(ssn)!=null) {
			System.out.println(BankService.getInstance().getCustomer(ssn).getName()+","+BankService.getInstance().getCustomer(ssn).getAddress().getAddressInfo());
		}else return;
